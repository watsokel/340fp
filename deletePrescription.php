...
if(!isset($_POST['selectPatient']) || empty($_POST['selectPatient']) || ("" == trim($_POST['selectPatient']))) {
  echo "Unable to delete prescription. You must select a patient.<br>";
} else {
...
?>
    ...
    <section>
      ...
      <form action="deletePrescription2.php" method="post">
        <fieldset>
        <legend>Delete Medication from this patient</legend>
        <table>
          <tr>
            ...
            <?php
            if(!($stmt = $mysqli->prepare("SELECT " . "me.name 
            FROM patients pt
            LEFT JOIN takes ti ON pID = patient_ID 
            LEFT JOIN medications me ON me.name = ti.drug_name WHERE pID=$_POST[selectPatient]"))){
              echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
            }
            if(!$stmt->execute()){
              echo "Execute failed: "  . $mysqli->connect_errno . " " . $mysqli->connect_error;
          ...
          <tr>
            <td><input type="submit" value="Delete Medication from this patient"></td>
          </tr> 
        </table>
        </fieldset>
      </form>
  ...
  Cancel and return to <a href="prescription.php">Prescription</a> page
  </body>
</html>