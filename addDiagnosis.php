...
if(!isset($_POST['selectPatient']) || empty($_POST['selectPatient']) || ("" == trim($_POST['selectPatient']))) {
  echo "Unable to add diagnosis. You must select a patient.<br>";
} else {
  echo '<table border="1" id="databaseData"><caption><h2>Current Diagnoses for this Patient</h2></caption>
        <thead>
          <tr>
...
?>
    ...
    <section>
      <h2>Add Diagnosis</h2>
      <form action="addDiagnosis2.php" method="post">
        <fieldset>
        <legend>Add Medical Condition to this patient</legend>
        <table>
          <tr>
            ...
            <?php
            if(!($stmt = $mysqli->prepare("SELECT name FROM medical_conditions"))){
              echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
            }
            if(!$stmt->execute()){
              echo "Execute failed: "  . $mysqli->connect_errno . " " . $mysqli->connect_error;
          ...
          <tr>
            <td><input type="submit" value="Add Condition to this Patient"></td>
          </tr> 
        </table>
        </fieldset>
      </form>
